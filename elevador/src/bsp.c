...
void *udpServer() {
    ...
    while (1) {
        recv_len = recvfrom(ss, buf, BUFLEN, 0, (struct sockaddr *)&si_other, &slen);
        if (recv_len == -1) {
...
void BSP_init(int argc, char *argv[]) {
    ...
    if (pthread_create(&tudpServer, NULL, udpServer, NULL) != 0) {
